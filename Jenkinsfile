pipeline {
    agent any
     ...
     stages {
        stage('git clone') {
            steps {
                git branch: 'main', url: 'https://github.com/Tejaswi53/Ekart.git'
            }
        }
                ...
                //withCredentials([aws(accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'aws', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY')]) {
                 // some block
                  ...
                  script {
                    ...
                    docker tag ekart:latest 987014986560.dkr.ecr.us-west-1.amazonaws.com/ekart:latest
                    docker push 987014986560.dkr.ecr.us-west-1.amazonaws.com/ekart:latest
                    '''
                  }
        ...
        stage('integrating eks cluster') {
            steps {
                withKubeCredentials(kubectlCredentials: [[caCertificate: '', clusterName: 'ekart', contextName: '', credentialsId: 'ekart-eks', namespace: 'ekart', serverUrl: 'https://3A7762C6F8EFF0BB1F35254CC19CE5F4.sk1.us-west-1.eks.amazonaws.com']]) {
                 // some block
                  script {
                    sh ' kubectl get ns '
